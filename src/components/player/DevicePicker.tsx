...
export function DevicePicker() {
  ...
  const open = usePlayerStore((s) => s.deviceMenuOpen);
  const setOpen = usePlayerStore((s) => s.setDeviceMenuOpen);
    ...
    <div className="relative">
      <button
        ...
        className={cn(
          ...
          playbackError
            ? 'animate-pulse border-signal-500 text-signal-400'
            : activeDevice
      ...
      {open && (
        ...
        >
          ...
          {playbackError && (
            <p className="px-2 pb-1.5 text-xs text-signal-400">{playbackError}</p>
          )}
          {devices.length === 0 && (
