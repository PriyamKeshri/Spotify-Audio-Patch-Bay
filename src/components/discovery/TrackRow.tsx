import { GripVertical, Heart, ListX, MoreHorizontal, Pause, Play, Plus } from 'lucide-react';
...
interface TrackRowProps {
  ...
  onRemove?: () => void;
  showAlbum?: boolean;
  // Drag-to-reorder — all optional, only wired up by pages that own an
  // editable ordering (currently just PlaylistPage for the owner's tracks).
  draggable?: boolean;
  isDragOver?: boolean;
  ...
  return (
    <div
      ...
      className={cn(
        ...
        isDragOver && 'outline outline-2 outline-signal-500/60'
      )}
    >
              ...
              >
                ...
                {onRemove && (
                  <button
                    ...
                    className="flex w-full items-center gap-2 rounded-md px-3 py-2 text-left text-xs text-alert-500 hover:bg-console-700"
                  >
                    <ListX className="h-3.5 w-3.5" /> Remove from playlist
                  </button>
                )}
              </div>
            )}
