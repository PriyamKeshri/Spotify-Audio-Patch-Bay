...
/**
 * All /me/player/* write endpoints (play, pause, seek, volume, shuffle,
 * repeat, queue…) 404 with reason NO_ACTIVE_DEVICE when there's no Spotify
 * Connect device currently active — this is expected, not a bug, and the
 * UI should prompt for a device rather than surface it as a hard failure.
 */
export function isNoActiveDeviceError(error: unknown): boolean {
  ...
  const details = error.details as SpotifyApiErrorShape | undefined;
  return details?.error?.reason === 'NO_ACTIVE_DEVICE' || /no active device/i.test(error.message);
}
