...
export async function saveTracks(ids: string[]): Promise<void> {
  await spotifyClient.put('/me/library', { uris: ids.map((id) => `spotify:track:${id}`) });
}

export async function removeSavedTracks(ids: string[]): Promise<void> {
  await spotifyClient.delete('/me/library', {
    data: { uris: ids.map((id) => `spotify:track:${id}`) },
  });
}
...
export async function saveAlbums(ids: string[]): Promise<void> {
  await spotifyClient.put('/me/library', { uris: ids.map((id) => `spotify:album:${id}`) });
}

export async function removeSavedAlbums(ids: string[]): Promise<void> {
  await spotifyClient.delete('/me/library', {
    data: { uris: ids.map((id) => `spotify:album:${id}`) },
  });
}
...
export async function followArtists(ids: string[]): Promise<void> {
  await spotifyClient.put('/me/library', { uris: ids.map((id) => `spotify:artist:${id}`) });
}

export async function unfollowArtists(ids: string[]): Promise<void> {
  await spotifyClient.delete('/me/library', {
    data: { uris: ids.map((id) => `spotify:artist:${id}`) },
  });
}
