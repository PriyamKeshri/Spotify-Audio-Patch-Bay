...
export interface SpotifyPlaylistTrack {
  added_at: string;
  // Renamed from `track` -> `item` when Spotify's Feb 2026 changes moved
  // this endpoint from /playlists/{id}/tracks to /playlists/{id}/items.
  item: SpotifyTrack;
}
...
export interface SpotifyApiErrorShape {
  error: {
    status: number;
    message: string;
    // Only present on some player endpoints, e.g. "NO_ACTIVE_DEVICE" /
    // "PREMIUM_REQUIRED" — not part of Spotify's general error envelope.
    reason?: string;
  };
}
